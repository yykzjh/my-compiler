...
public:
    ...
    void strToNFA(string s, int id)
    {
        ...
        for (int i = 0; i < (int)s.size(); ++i)
        {
            ...
            else if (ch == '*') { //闭包运算，把外层（st栈顶和此栈顶）的起点和终点用epsilon连接起来。
                int nxt = s_st.top();
                s_st.pop();
                int pre = s_st.top();
    ...
    void outputMinDFA(ofstream &fout) {
        fout << mDfaId << endl;
        for (int i = 0; i < mDfaId; i++) {
            for (int j = 0; j <= 257; j++) {
                if (mdf[i][j] != -1) {
                    fout << i << ' ' << j << ' ' << mdf[i][j] << endl;
                }
            }
        }
        fout<<-1<<endl;
...
public:
    vector<string> Type;
    void addstr(string &name,string &str, int &id) {
        Type.push_back(name);
        id = Type.size() - 1;
        string new_s;
        for (int i=0; i < (int)str.size(); i++) {
            if (str[i] == '\\') new_s += str[i++], new_s += str[i];
            else if (str[i] == '[') {
                for (int j = str[i + 1]; j < str[i + 3]; j++)
                    new_s += j, new_s += '|';
                new_s += str[i + 3];
                i += 4;
            }
...
int main()
{
	ifstream fin("regular2minDFA-input.txt");
	ofstream fout("analysis-dfaMap-input.txt");
